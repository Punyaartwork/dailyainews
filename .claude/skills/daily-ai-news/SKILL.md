---
name: daily-ai-news
description: Generate a daily Thai-language AI news brief covering up to 5 stories from the last 24 hours (rolling window, Asia/Bangkok), with URLs deduplicated against yesterday's brief so the same story is never covered twice in a row, enrich it with three expert perspectives (professor / AI specialist / professional programmer), commit it to this repository via the GitHub connector, and push a TL;DR to Telegram. Use this when the user asks for a "daily AI news brief", a "สรุปข่าว AI วันนี้", triggers this skill by name, or when it runs on schedule via Claude Web Routine.
---

# Daily AI News Brief

End-to-end routine that produces **one Markdown article per day** at `articles/YYYY-MM-DD-brief.md`, commits it via the GitHub connector, then notifies Telegram. Runs entirely inside Claude (Web / Routine) — **no shell, no git CLI, no Bash**.
...
The Routine itself needs very little env now — Telegram moved out to GitHub Actions (see Step 6). The skill reads only:
...
`TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` are **no longer read by the skill** — they live in GitHub repo secrets and are consumed by `.github/workflows/telegram-notify.yml`. If you still see LINE vars in an older prompt, ignore them; the Routine doesn't need them.

Claude Web Routine's Cloud Environment feature does not reliably inject values into the model's prompt context (observed empirically in this deployment). That's fine — GitHub vars have a committed defaults fallback, and Telegram secrets now live on a surface (GH secrets) that actually works.
...
Save the final markdown as `ARTICLE_BODY`.

**Commit checkpoint — commit working files NOW before proceeding:**
1. Commit `reference/sources.md` via GitHub connector (use SHA from Step 1d if updating).
2. Commit `reference/perspectives.md` via GitHub connector (use SHA from Step 3 if updating).
3. Only after both commits succeed → proceed to Step 5.

This splits the work into smaller MCP calls and prevents stream idle timeout
on long article generation.
---
...
- **Found, content byte-for-byte identical to `ARTICLE_BODY`.** Skip the commit. Log: `Run status: NO-OP (idempotent) — today's brief already committed at <existing SHA>`. Capture that SHA as `COMMIT_SHA` and **still proceed to Step 6** (Telegram may have been skipped last time).
...
5. If the commit call fails: print the full connector error, do not retry silently, do not proceed to Telegram.

---

## Step 6 — Telegram push (handled externally)

**This skill does NOT send to Telegram.** The `WebFetch` tool in Claude Web Routine accepts only `(url, prompt)` — no method, no headers, no body — so it cannot POST to `api.telegram.org/bot.../sendMessage` with a Bearer token. That's a tool-schema limit, not an egress or env issue.

Telegram dispatch is instead handled by a GitHub Actions workflow at [`.github/workflows/telegram-notify.yml`](../../../.github/workflows/telegram-notify.yml) that triggers on every push to `main` affecting `articles/*-brief.md`. The workflow:

- Reads repo secrets `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID`.
- Extracts the title + TL;DR bullets from the newly committed brief.
- Builds a permalink pinned to the commit SHA.
- POSTs to `api.telegram.org` with `curl` (full HTTP capability).
- Fails loudly in the Actions UI on non-200.

What the skill does here:

1. Do nothing that talks to Telegram directly — no `WebFetch`, no markers, no retries.
2. Print one line for the operator: `Telegram: dispatched by .github/workflows/telegram-notify.yml (see Actions tab for delivery status)`.
...
If the maintainer wants to force a re-notify for an already-committed brief, they can run the workflow manually via `gh workflow run telegram-notify.yml -f file=articles/{TODAY}-brief.md` or the GitHub UI.
...
✅ Committed: articles/{TODAY}-brief.md @ {COMMIT_SHA_short}
   {PERMALINK}
Telegram: dispatched by .github/workflows/telegram-notify.yml (see Actions tab for delivery status)
```
...
Run status: NO-OP (idempotent) — today's brief already at {COMMIT_SHA_short}
   {PERMALINK}
Telegram: not re-notified (no new commit → workflow does not fire)
```
...
| Today's article already committed, content identical | NO-OP. Telegram workflow does not fire (intentional). |
| Today's article already committed, content differs | Update via the returned SHA. Telegram workflow will fire on the new push. |
| Telegram issues | Not this skill's concern — see `.github/workflows/telegram-notify.yml` and the Actions tab. |
